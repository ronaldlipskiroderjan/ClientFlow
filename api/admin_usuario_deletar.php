...
if ($usuario_tipo !== 'admin') {
    $retorno["mensagem"] = "Perfil sem permissão para esta ação.";
    header("Content-type: application/json;charset:utf-8");
    echo json_encode($retorno);
    exit();
}
...
if (empty($usuario_id_excluir)) {
    $retorno["mensagem"] = "ID do usuário não fornecido.";
    header("Content-type: application/json;charset:utf-8");
    echo json_encode($retorno);
    exit();
}
...
if ($usuario_id === $usuario_id_excluir) {
    $retorno["mensagem"] = "Você não pode deletar a sua própria conta.";
    header("Content-type: application/json;charset:utf-8");
    echo json_encode($retorno);
    exit();
}
...
try {
    ...
    if (!$stmt_delete_ua->execute()) {
        throw new Exception("Erro ao deletar vínculos de agência");
    }
